...
public class RageSpline : MonoBehaviour, IRageSpline
{
	...
	public IRageTriangulator Triangulator
	{
		get
		{
			if (_triangulator == null) _triangulator = ScriptableObject.CreateInstance<RageTriangulator>();
	...
	private RageVertex[] OutlineVerts (bool antialiasing, bool multipleMaterials) {
		...
		for (int v = 0; v < splits.Length; v++) {
			...
			var outlineNormalOffset = GetOutlineNormalOffset();

			outlineVerts = OutlineVertsProcessSplit(antialiasing, scaledNormal, vertsInBand, edgeWidth, outlineNormalOffset, normalizedNormal, outlineVerts, v, 
													...
													Vector3 normalizedNormal, RageVertex[] outlineVerts, int v, Vector3 normal, RageVertex[] splits, float AAWidth) 
	{
		...
		if (antialiasing) {
			...
			outlineVerts[v + 0 * vertsInBand].position = Mathfx.Add(splits[v].position, normalizedAaWidth, normalizedEdgeWidth, normalizedOutlineNormal, freeLineCapTangent);
			...
			outlineVerts[v + 3 * vertsInBand].position = splits[v].position - normalizedAaWidth - normalizedEdgeWidth + Mathfx.Add(normalizedOutlineNormal, freeLineCapTangent);
		} else {
