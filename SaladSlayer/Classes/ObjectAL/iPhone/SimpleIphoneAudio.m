...
- (NSUInteger) preloadCacheSize
{
	OPTIONALLY_SYNCHRONIZED(self)
	{
		NSUInteger totalSize = 0;
		for (NSString *key in [preloadCache allKeys]) {
			ALBuffer *buffer = [preloadCache objectForKey:key];
...
- (ALBuffer*) internalPreloadEffect:(NSString*) filePath
{
	...
	if(nil == buffer)
	{
		//DebugLog(@"%s cached buffer for %@", __PRETTY_FUNCTION__, filePath);
	...
	{
		//DebugLog(@"%s used buffer cache for %@", __PRETTY_FUNCTION__, filePath);
