...
-(void) initializeUIKit {
	...
	[scoresButton setImage: isFullGame ? [UIImage imageNamed:@"btnGameOverScores.png"] : [UIImage imageNamed:@"btnBuyNowSmall.png"] forState:UIControlStateNormal];
...
-(void) animateGameOverIn {
	...
	if (isFullGame) {
		if (mainGame.score > [[GameKitLibrary sharedGameKit] getBestScoreForCategory: mainGame.mode])
	...
	newHighScore.hidden = !highscoreVisible;
		...
		if ([GameKitLibrary sharedGameKit].gameCenterAvailable) {
			...
			if ([GameKitLibrary sharedGameKit].internetConnectionAvailable) {
				...
				if ([GameKitLibrary sharedGameKit].gameCenterPlayerLoggedIn) {
					FruitSlayerAppDelegate *delegate = (FruitSlayerAppDelegate *) [[UIApplication sharedApplication] delegate];
					[[GameKitLibrary sharedGameKit] showLeaderboardOver:delegate.menuViewController Category:mainGame.mode Timescope:0];
				} else {
...
-(void) gameOver {
	...
	if (isFullGame) {
		...
		if ([GameKitLibrary sharedGameKit].gameCenterAvailable && [GameKitLibrary sharedGameKit].gameCenterPlayerLoggedIn)
			[[GameKitLibrary sharedGameKit] syncAll];
		else
